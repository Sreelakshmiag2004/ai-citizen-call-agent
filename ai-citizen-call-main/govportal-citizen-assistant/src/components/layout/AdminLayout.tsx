...
export const AdminLayout: React.FC<AdminLayoutProps> = ({ 
  ...
  const isNavActive = (route: PageRoute) => {
    ...
    if (route === 'admin-notifications' && currentRoute === 'notifications') return true;
    if (route === 'admin-profile' && currentRoute === 'profile') return true;
    return false;
  };

  return (
    <div id="admin-shell" className="min-h-screen bg-[#F8FAFC] flex overflow-x-hidden font-sans">
      {/* Dark Navy Sidebar */}
      <aside 
        id="admin-sidebar"
      ...
      >
        ...
        >
          ...
          {!isSidebarCollapsed && (
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-[11px] text-slate-400 font-medium tracking-wide mt-1">Admin Portal</span>
            </div>
          )}
        </div>

        {/* Navigation Items */}
        <nav id="admin-nav" className="flex-1 px-3 py-6 space-y-1.5 overflow-y-auto">
          {navItems.map((item) => {
            const active = isNavActive(item.route);
            return (
              <button
                key={item.route}
                id={`admin-nav-${item.route}`}
                onClick={() => navigate(item.route)}
                title={isSidebarCollapsed ? item.label : undefined}
                className={`w-full flex items-center rounded-xl text-sm font-medium transition-all duration-150 relative ${
              ...
              >
                ...
                {item.badge && !isSidebarCollapsed && (
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${
                    active ? 'bg-white text-[#1D4ED8]' : 'bg-[#E11D48] text-white'
                  }`}>
                    {item.badge}
                  </span>
                )}
        ...
        <div id="admin-sidebar-footer" className="p-3 border-t border-slate-800/80">
          <button
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-screen overflow-y-auto">
        {/* Top Header Bar */}
        <header id="admin-top-bar" className="bg-white border-b border-slate-200/80 px-6 py-4 flex items-center justify-between sticky top-0 z-20 shadow-2xs">
          {/* Left Title / Subtitle */}
          <div>
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight leading-snug">
              {pageTitle || (currentRoute === 'dashboard' ? 'Welcome, Raj Kumar 👋' : 'GovPortal')}
            </h1>
            <p className="text-xs text-slate-500 font-normal mt-0.5">
              {pageSubtitle || (currentRoute === 'dashboard' ? "Here's the overall system overview across GovPortal." : 'Administrator Workspace')}
            </p>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              ...
              >
                <div className="w-9 h-9 rounded-full bg-slate-900 text-white font-bold text-xs flex items-center justify-center border border-slate-300">
                  {user?.avatarInitials || 'RK'}
                </div>
                <div className="text-left hidden sm:block">
                  <div className="text-xs font-bold text-slate-800 leading-tight">
                    {user?.fullName || 'Raj Kumar'}
                  </div>
              ...
              {isProfileMenuOpen && (
                ...
                >
                  <div className="px-4 py-2.5 border-b border-slate-100">
                    <p className="text-xs font-semibold text-slate-900">{user?.fullName || 'Raj Kumar'}</p>
                    <p className="text-[11px] text-slate-500">{user?.email || 'raj.kumar@gov.in'}</p>
                  </div>
                  ...
                  <div className="py-1">
                    ...
                    <button
                      onClick={() => { setIsProfileMenuOpen(false); navigate('admin-notifications'); }}
                      className="w-full px-4 py-2 text-left text-xs text-slate-700 hover:bg-slate-50 flex items-center gap-2.5"
                    >
